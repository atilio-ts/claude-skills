...
| [`/scaffold-feature`](./scaffold-feature/SKILL.md) | Scaffolds a new feature or integration port for a Spring Boot Clean Architecture project on the `api-projectbase-spring` template (CleanArchBeanRegistry DI, ArchUnit layering), auto-detecting the base package so it works unmodified across sibling OCA repos. |
